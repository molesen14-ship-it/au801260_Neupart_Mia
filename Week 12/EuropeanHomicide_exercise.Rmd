...
Answer: It could be because of general stability in the country

```{r eval = FALSE}
# YOUR CODE HERE:

Den_Danske_Kongerække <-read_csv2("data/Kings-kopi.csv")
Den_Danske_Kongerække %>% 
  mutate(duration = reign_end - reign_start) %>%   
  mutate(Midyear = reign_end - duration/2) %>%  
  ggplot(aes(x = Midyear, y = duration))+
  geom_point(color = "blue", size =kings$duration)+
  geom_smooth()+    
  theme_classic() 
